...
{"country":"Czechia","value":1,"rus":"Чехия"},
...
{"country":"Hungary","value":1,"rus":"Венгрия"},
...
{"country":"Turkey","value":1,"rus":"Турция"},
...
function makeWorldMap(id,w) {
    ...
    var keys = ['Другие страны',"Страна НАТО", "Украина", "Россия"]
    svg.selectAll("mydots")
        .data(keys)
        .enter()
        .append("rect")
            .attr("x", 0)
            .attr("y", function(d,i){ return 122 + i*18}) // 100 is where the first dot appears. 25 is the distance between dots
            .attr("width", 10)
            .attr("height", 10)
    ...
    .append("text")
        .attr("x", 15)
        .attr("y", function(d,i){ return 130 + i*18.5}) // 100 is where the first dot appears. 25 is the distance between dots
        .style("fill", 'black')
    ...
    var pointsText = ['Москва',"Киев","Рига","Таллин","Вильнюс",'Нейден, Норвегия']
    ...
    d3.xml("https://cdn.jsdelivr.net/gh/liga-net/few-projects/balloon.svg", function(xml) {  
        ...
        svg.selectAll("g")
            ...
            .attr("transform", function(d, i){ 
                return "translate(" + (projection(d[0])[0]-15) + "," + (projection(d[0])[1]-10)   + ")"+"scale("+ scale +")";
            })
            .each(function(d, i){ 
        ...
        var balloons = d3.selectAll('.balloons')
            ...
            .duration(function(d,i) {return time[i]/50})
            .attr("transform", function(d, i){ 
                return "translate(" + (projection(d[1])[0]-15) + "," + (projection(d[1])[1]-10)   + ")"+"scale("+ scale +")";
            })
        })
    ...
    counties_list = ['Польша', "Чехия", "Венгрия", "Турция", "Румыния", "Испания", "Германия"]
    svg.selectAll("text")
        .data(topojson.feature(world, world.objects.countries).features.filter(d => d.properties.admin != 'Antarctica'))
        .enter()
        .append("svg:text")
