...
var bavovna = [{'type': 'Feature','geometry': {'type': 'Point', 'coordinates': [37.3855235, 55.582026]},'name': 'Москва','region': 'Місто','days': '2023-02-28','balance': '','file':'city.svg'},
...
function makeMap(w,json) {
    ...
    var points = cities_mer.selectAll(".point")
        .data(bavovna.filter(d => d.days == '2023-02-28'))
        .enter()
        .append("image")
        ...
        .attr("xlink:href", function(d) { return 'SVG/'+d.file; })
...
var startDate = new Date("2023-02-28");
...
d3.select("#value-time").text("Бавовна в Росії з 28 лютого 2023: "+bavovna.filter(d => new Date(d.days) <= new Date(value)).length)
});
...
d3.select("#value-time").text('Кількість бавовн в Росії з 28 лютого');
