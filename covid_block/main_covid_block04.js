...
var config = [
    ...
    {id: 'vaccination','url':'https://www.liga.net/files/general/data/coronavirus/11.json',
...
function createVaxNumberTimeline(column,data) {
    var AstraZeneca = data.daily[column].AstraZeneca
    var sinovac = data.daily[column]['Sinovac Biotech']
...
function fillNumbers(dict) {
    ...
    $.getJSON(requestURL, function(data) {
        if (dict.id == 'api-covid') {
            ...
            var myDashboard = {
                "vax_day" : createVaxNumberTimeline('quantity',data).slice(-1)[0],
                "vax_day_spark": createVaxNumberTimeline('quantity',data).slice(-6),
                "vax_all" : createVaxNumberTimeline('cumulative',data).slice(-1)[0]
            }
        }else if (dict.id == 'hospitalization'){
            var myDashboard = {
                "hosp_day":data.count.i1.slice(-1)[0],
                "hosp_day_spark":makeDailyStatistics(data.count.i1.slice(-6)),
                "hosp_all":data.count.i1.slice(-1)[0]-data.count.i1.slice(-2)[0]
            }
