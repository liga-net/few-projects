...
function makeChoroplethMap() {
    ...
    var tooltipBee = d3.select("body")
        ...
        if (responseJson == undefined || zero.length > 0 ) {
            var yesterday = new Date()
            yesterday.setDate(yesterday.getDate() - 1) 
            var responseJson = $.ajax({ 
                url: config[3].url+todayDateFormat(yesterday),
                async: false
            }).responseJSON;
        }
        var ukraine = $.ajax({ 
...
var config = [
    {id: 'api-covid','url':'https://api-covid19.rnbo.gov.ua/charts/main-data?mode=ukraine',
    table_ids:["date","ill_all","ill_day","dead_all","dead_day"]},
    {id: 'hospitalization','url':'https://health-security.rnbo.gov.ua/api/beds/hospitalization/chart?bedType=hospitalized&regionId=&hospitalType=',
    table_ids:["hosp_day",'hosp_all']},
    {id: 'vaccination','url':'https://health-security.rnbo.gov.ua/api/vaccination/process/chart?vaccines=&dose=1&distributionBy=vaccine&regionId=',
    table_ids:["vax_day",'vax_all']},
    {id: 'regions','url':'https://api-covid19.rnbo.gov.ua/data?to=',
    table_ids:['region0_day','region0_all','region0_text','region1_day','region1_all','region1_text','region2_day','region2_all','region2_text']}
]
...
function fillNumbers(dict) {
    ...
    $.getJSON(requestURL, function(data) {
        if (dict.id == 'api-covid') {
            if (data.confirmed.slice(-1)[0] - data.confirmed.slice(-2)[0] == 0) {
