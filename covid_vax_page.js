if(document.createStyleSheet) {
    document.createStyleSheet('https://cdn.jsdelivr.net/gh/liga-net/few-projects/covid_vax_page.css');
   }
   else {
    var styles = "@import url('https://cdn.jsdelivr.net/gh/liga-net/few-projects/covid_vax_page.css');";
...
function makeBarChart(id,w) {
        ...
        if (id == 'confirmed') {
            ...
            var json = timeline_all.map(d => parseFloat(d['gsx$susp']['$t']))
            var x_date = timeline_all.filter(d => d['gsx$susp']['$t'] != '').map(d => d['gsx$date']['$t'])
            var plusMax = 5000
            var color = '#4caf50'
            var tooltipText = ' тестов за сутки</p>'
            var importedTickValues = x_date.filter(function(d,i) {if (i%50 == 0) {return d}})
        } else if (id=='hosp'){
...
function makeChoroplethMap(id,w,red_zones_data) {
        ...
        if (id != 'red_zones') {
            if (id == 'vaccinated') {
                var json = []
                regionsVaxData.forEach(function(d,i) {
                    json.push({
                        'region':d.properties.name,
                        'value':d.properties.daily.total.cumulative
                    })
                })
            } else if (id == 'reserves') {
                var json = []
            ...
            } else if (id == 'vaccinatedPer10k') {
                var json = []
                regionsVaxData.forEach(function(d,i) {
                    json.push({
                        'region':d.properties.name,
                        'value':d.properties.vaccinatedPer10k
                    })
                })
            }
        ...
        g.selectAll("text")
            ...
            if (id != 'red_zones') {
                ...
                cities_mer.selectAll("circle")
                    ...
                    .on("mouseover", function(d) {
                        if (id == 'vaccinated') {
                            tooltipBee.html('<h4>Киев</h4><p>Вакцинированно '+numberWithSpaces(kyiv[0].value)+' человек</p>')
                        } else if (id == 'reserves'){
...
function createVaxNumberTimeline(column) {
    var AstraZeneca = timelineVaxData.daily[column].AstraZeneca
    var sinovac = timelineVaxData.daily[column]['Sinovac Biotech']
    var json = AstraZeneca.map(function(d,i) {
        return d + sinovac[i]
    })
    return json
...
if (covid_timeline.confirmed.slice(-1)[0] - covid_timeline.confirmed.slice(-2)[0] == 0) {
    ...
    var myDashboard = {
        ...
        "vaxtoday" : createVaxNumberTimeline('quantity').slice(-1)[0],
        "allvax" : createVaxNumberTimeline('cumulative').slice(-1)[0]
    }
} else {
    var myDashboard = {
...
var table_ids = ["date","allill","illtoday","alldied","diedtoday","hosp","allvax","vaxtoday"]

table_ids.forEach(function(i) {
    addDashboard(i)
})

var timeline_all = getData(rnboLinks.timeline)['feed']['entry'];
timeline_all.forEach(function(d) {
    d['gsx$date']['$t'] = parseTime(d['gsx$date']['$t']);
}); 

makeBarChart('confirmed',window.innerWidth) 

makeChoroplethMap('red_zones',window.innerWidth,timeline_all)
makeBarChart('tests',window.innerWidth) 
makeBarChart('deaths',window.innerWidth)
makeBarChart('hosp',window.innerWidth)
makeMultiLineChart(window.innerWidth,'lizh')
makeBarChart('discharged',window.innerWidth)

var regions = timeline_all.map(d => d['gsx$name']['$t']).slice(0,25)
if (window.innerWidth > 600) {
    for (i in regions) {
        makeNegaviveBarChart(190,'reg'+String(i),regions[i])
    }
} else {
    for (i=0;i<6;i++) {
        makeNegaviveBarChart(170,'reg'+String(i),regions[i])
    }
}
makeBarChart('vax_day',window.innerWidth)
makeChoroplethMap('vaccinated',window.innerWidth)
makeChoroplethMap('vaccinatedPer10k',window.innerWidth)
var vax_regions_daily = regionsVaxData.filter(d => d.properties.daily.total.quantity != 0)
if (vax_regions_daily.length > 0) {
    makeChoroplethMap('vax_regions_daily',window.innerWidth)
    d3.select('#vax_regions_daily_text').style('display','block')
}
makeChoroplethMap('reserves',window.innerWidth)
makeColumnChart('vax_name',window.innerWidth)
makeColumnChart('vax_age',window.innerWidth)
makeColumnChart('vax_group',window.innerWidth)
makeMultiLineChart(window.innerWidth,'temps50')
