...
function makeColumnChart(id, w) { 
    ...
    if (id == 'team_revenue') {
        ...
        var date = ["Італія", "Англія", "Іспанія", "Данія", "Бельгія", "Чехія", "Швейцарія", "Україна", "Нідерланди", "Швеція", "Австрія", "Франція", "Німеччина","Португалія","Уельс","Хорватія","Угорщина","Росія","Словаччина","Фінляндія","Польща","Шотландія","Македонія","Туреччина"]
        ...
        var left = 170
        var id_height = 600
        var title = "Скільки збірні вже заробили за Євро (не враховуючи фінал)"
    } else if (id == 'team_price') {
        var change_format = d3.format("")
        var date = ['Англія', 'Франція', 'Іспанія', 'Німеччина', 'Португалія', 'Італія', 'Бельгія', 'Нідерланди', 'Хорватія', 'Туреччина']
        var json = [1263000000, 1028000000, 915000000, 896500000, 827500000, 751000000, 664400000, 607050000, 376800000, 325000000]
        var tooltipHtml = '<p>'
        var left = 170
        var id_height = 420
        var title = 'Сумарна вартість футболістів збірної'
    }
    if (start_width > 500) {
        var tickValue = 7
        var fontSize = '16px'
...
function makeBarChart(id,w) {
    ...
    svg.append("text")
        ...
        .style("font-size", fontSize)
        .style("font-family", "Georgia")
        .text("Відвідуваність стадіонів на чемпіонатах Європи");
