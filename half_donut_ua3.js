...
var fullData = [
    {"name": "Дизель","Споживання": 7446.6,"Імпорт з Росії": 2613.9},
    {"name": "Бензин","Споживання": 2143.8,"Імпорт з Росії": 0},
    {"name": "Автогаз","Споживання": 1982.3,"Імпорт з Росії": 689.5},
    {"name": "Вугілля","Споживання": 380003061,"Імпорт з Росії": 11872893}]
...
svgLegend.append("text").attr("x", 20).attr("y", 19).text("Споживання").style("font-size", "14px").attr("alignment-baseline","left")
svgLegend.append("text").attr("x", 20).attr("y", 35).text("Імпорт з Росії").style("font-size", "14px").attr("alignment-baseline","left")
var full_data = fullData[number]
var text = full_data['name']
var data = [full_data['Споживання'],full_data['Імпорт з Росії']]
...
.on("mouseover", function(d,i) {
    ...
    if (name == 'Імпорт з Росії') {
        var perc = Math.round(d.data / full_data['Споживання'] * 100)
        tooltipBee.html("<h4>"+text+'</h4><p>'+name+': '+numberWithSpaces(Math.round(d.data))+' тис. тон ('+perc+'%)</p>')
    } else {
        tooltipBee.html("<h4>"+text+'</h4><p>'+name+': '+numberWithSpaces(Math.round(d.data))+' тис. тон</p>')
    }
