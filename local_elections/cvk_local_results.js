...
function getData(id){
    var value = $.ajax({ 
        url: 'https://cdn.jsdelivr.net/gh/liga-net/few-projects@1211/local_elections/'+id+'.json', 
        async: false
    }).responseJSON;
    return value;
}
...
function makeTable(scraped, num) {
    ...
    if (num == 0) {
        ...
        tr.innerHTML = '<th style="text-align:left;">Область</th>' +
                                '<th>Партия №1</th>'+'<th>Партия №2</th>'+'<th>Партия №3</th>' ;
    } else if (num == 1) {
        var fillValue = scraped.features.filter(a=>$.isEmptyObject(a.results) == false)
        ...
        tr.innerHTML = '<th style="text-align:left;"Город</th>' +
                                '<th>Кандидат №1</th>'+'<th>Кандидат №2</th>';
    } else {
        var fillValue = scraped.data.filter(a=>$.isEmptyObject(a.properties.results) == false)
        tr.innerHTML = '<th style="text-align:left;">Городской совет</th>' +
                                '<th>Партия №1</th>'+'<th>Партия №2</th>'+'<th>Партия №3</th>' 
    }
    ...
    for (r=0;r < fillValue.length; r++) {
        ...
        if (num == 0) {
            tr.innerHTML = '<td>' + fillValue[r].name + '</td>' +
                            '<td>' + fillValue[r].results[1].party +'<br>('+fillValue[r].results[1].places  + ' мест)</td>'+
                            '<td>' + fillValue[r].results[2].party +'<br>('+fillValue[r].results[2].places  + ' мест)</td>'
                    if (fillValue[r].results[3] != undefined){
                        tr.innerHTML +='<td>' + fillValue[r].results[3].party +'<br>('+fillValue[r].results[3].places  + ' мест)</td>'  
                    }
        } else if (num == 1) {
            tr.innerHTML = '<td>' + fillValue[r].name + '</td>' +
            ...
            tr.innerHTML = '<td>' + fillValue[r].properties.results[0].rada + '</td>' +
                            '<td>' + fillValue[r].properties.results[0].party +'<br>('+fillValue[r].properties.results[0].places  + ' мест)</td>'+
                            '<td>' + fillValue[r].properties.results[1].party +'<br>('+fillValue[r].properties.results[1].places  + ' мест)</td>'
                    if (fillValue[r].properties.results[2] != undefined){
                        tr.innerHTML +='<td>' + fillValue[r].properties.results['2'].party +'<br>('+fillValue[r].properties.results['2'].places  + ' мест)</td>'  
                    }
