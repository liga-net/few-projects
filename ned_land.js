...
if (id == 'quantity') {
    var title = 'Количество проданных земельных участков'
    var tooltipEnd = ' участков</p>'
} else if (id == 'square') {
    var title = 'Площадь проданных земельных участков'
    var tooltipEnd = ' га</p>'
} else if (id == 'avgPrice') {
    var title = 'Средняя цена за 1 га'
...
g.selectAll("path")
    ...
    .on("mouseover", function(d) {
        if (d.properties.name == 'Автономна Республіка Крим'){
            ...
            var html = '<h4>'+d.properties['name:ru']+'</h4><p>'+numberWithSpaces(items[0][id])+tooltipEnd
...
var locale = d3.timeFormatLocale({
    ...
    function makeBarChart(w,uploaded_data,id) {
        ...
        svg.append("text")
            ...
            .style("font-size", fontSize)
            .style("font-family", "Georgia")
            .text('Как менялась цена на аренду сельхоз земли за 1 га');
    ...
    function makeTable() {
        d3.select("#table")
            ...
            .html("<tr><td class='left'>Страна</td><td>Цена в начале либерализации</td><td>Через 10 лет</td><td></td></tr>")
            .selectAll('trd')
            .data(countryInfo)
            .enter()
            .append("tr")
    ...
    function makeSmallBarChart(country){
        ...
        var parseDate = d3.timeParse("%Y-%m-%d");
        var formatDay = locale.format("%Y")
        var tickValues = data.map(function(d) { return d.date; })/* .filter(function(d,i) {
            if (i == 1) {
                return d
            }
        }) */
        ...
        // Add Y axis
        var y = d3.scaleLinear()
        .domain([0, d3.max(data.map(function(d) { return d.vac; }))])
        .range([ height, 0]);
        /* svg.append("g")
        .call(d3.axisLeft(y)); */
