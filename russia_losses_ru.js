...
var dead_russians = [{'date': '26.02.2022', 'value': 2700},
 ...
 {'date': '24.08.2022', 'value': 3}]
    ...
    function makeAreaChart(id,data,w) {
        if (id == 'dead_russians') {
            var title = 'Убитые русские военные'
            var subtitle = ""
            var tooltip = ' убитых'
        } else if (id == 'bbm') {
            var title = 'Уничтоженые ББМ'
            var subtitle = ""
            var tooltip = ' уничтоженых ББМ'
        }else if (id == 'tanks') {
            var title = 'Уничтоженые танки'
            var subtitle = ""
            var tooltip = ' уничтоженых танков'
        }else if (id == 'artillery') {
            var title = 'Уничтоженые артиллерия'
            var subtitle = ""
            var tooltip = ' единиц уничтоженой артиллерии'
        }else if (id == 'jet') {
            var title = 'Уничтоженые самолеты'
            var subtitle = "Данные: Генштаб ВСУ"
            var tooltip = ' уничтоженых самолетов'
        }else if (id == 'helicopter') {
            var title = 'Уничтоженые вертолеты'
            var subtitle = ""
            var tooltip = ' уничтоженых вертолетов'
        }
        if (w < 600) {
            var widthW = w
        } else {
  ...
  if (w < 600) {
    ...
    //Read the data
    d3.csv("https://cdn.jsdelivr.net/gh/liga-net/few-projects/dangerous_time_ru.csv", function(data) {
    ...
    var tickGroup = myGroups.filter(function(d,i) { if (i % 240 == 0) {return d}})
      ...
      var mousemove = function(d) {
        tooltip
          .html("<h3>"+d.group+"</h3><p>" + d.value+' тревог</p>')
    ...
    svg.append("text")
            ...
            .style("fill", "grey")
            .style("max-width", 400)
            .text("Когда чаще всего звучат сирены, по областям");
