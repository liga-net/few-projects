if(document.createStyleSheet) {
    document.createStyleSheet('https://cdn.jsdelivr.net/gh/liga-net/few-projects/taxes_dashboard3.css');
}
else {
    var styles = "@import url('https://cdn.jsdelivr.net/gh/liga-net/few-projects/taxes_dashboard3.css');";
...
Highcharts.chart('chart_sankey', {
    ...
    tooltip: {
        formatter: function() {
            if (this.key.includes('highcharts')) {
                return '<b>'+this.point.from + '</b> направляют в ' +this.point.to.toLowerCase().replace('киева','Киева').replace('отг','ОТГ')+' на <b>'+this.point.percent+'%</b>'
            } else if (this.key.includes('Бюджет')) {
                return this.point.sum+' налогов поступает в '+this.key.toLowerCase().replace('киева','Киева').replace('отг','ОТГ')
            } else {
                if (this.point.sum ==1) {
                    return this.key+': направляется в '+this.point.sum+' бюджет'
                } else if (this.point.sum < 5) {
                    return this.key+': направляется в '+this.point.sum+' бюджета'
                } else {
    ...
    series: [{
        ...
        data: [
            ['НДФЛ', 'Бюджет Киева', 1,40],
            ['НДФЛ', 'Областные бюджеты', 1,15],
            ['НДФЛ', 'Бюджеты городов областного значения', 1,46],
            ['НДФЛ', 'Бюджеты ОТГ', 1,60],
            ['НДФЛ','Районные бюджеты',1,60],
        ...
        title: {
            text: 'Статьи расходов сведенного бюджета'
        },
        subtitle: {
            text: 'Данные: Cost.ua'
        },
    ...
    Highcharts.chart('debt', {
        ...
        title: {
            text: 'Крупнейшие налоговые должники'
        },
        subtitle: {
            text: 'Данные: ГНС'
        },
    ...
    Highcharts.chart('tax', {
        ...
        title: {
            text: 'Общая ставка налогов и взносов (% от прибыли)'
        },
        subtitle: {
            text: 'Данные: Doing Business'
        },
        xAxis: {
            categories: ["Дания","Германия","Польша","Латвия","Украина","Литва","Россия","Беларусь","Словакия"]
        },
        yAxis: {
            min: 0,
            title: null,
