...
Highcharts.chart('chart_sankey', {
    ...
    tooltip: {
        formatter: function() {
            if (this.key.includes('highcharts')) {
                return '<b>'+this.point.from + '</b> направляють у ' +this.point.to+' на <b>'+this.point.percent+'%</b>'
            } else if (this.key.includes('Бюджет')) {
                return this.point.sum+' податків надходить у '+this.key
            } else {
                if (this.point.sum ==1) {
    ...
    series: [{
        ...
        data: [
            ...
            ['ПДФО', 'бюджети міст обласного значения', 1,46],
        ...
        title: {
            text: 'Найбільші видатки бюджету'
        },
        subtitle: {
            text: 'Дані: Cost.ua'
        },
        xAxis: {
            categories: ['Освіта', 'Пенсії', 'Медицина', 'Правоохоронні органи', 'Відсотки по боргах', 'Транспорт', 'Соціальний захист', 'ЗСУ', 'Чиновники', 'Інше', 'Культура', 'Комунальне господарство','Сільське господарство','Охорона навколишнього середовища','Енергетика']
        },
        yAxis: {
            min: 0,
            title: null,
