    ...
    tooltip: {
        formatter: function() {
            if (this.key.includes('highcharts')) {
                return '<b>'+this.point.from + '</b> направляють у ' +this.point.to.toLowerCase().replace('киева','Киева').replace('отг','ОТГ')+' на <b>'+this.point.percent+'%</b>'
            } else if (this.key.includes('Бюджет')) {
                return this.point.sum+' податків надходить у '+this.key.toLowerCase().replace('киева','Киева').replace('отг','ОТГ')
            } else {
                if (this.point.sum ==1) {
