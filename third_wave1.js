...
var codesUkr = [{'name': 'robe_current', 'ukr': 'Халаты изоляционные одноразовые'},
    {'name': 'respirator_ffp3_current',
    'ukr': 'Респираторы класса FFP2'},
    {'name': 'mask_disposable_current',
    'ukr': 'Одноразовые хирургические маски'},
    {'name': 'mask_reusable_current',
    'ukr': 'Многоразовые хирургические маски'},
    {'name': 'glasses_disposable_current',
    'ukr': 'Одноразовые защитные щитки и очки'},
    {'name': 'glasses_reusable_current',
    'ukr': 'Многоразовые защитные щитки и очки'},
    {'name': 'gloves_current',
    'ukr': 'Перчатки нитриловые'},
    {'name': 'total_ivl_work', 'ukr': 'Общее количество исправных ИВЛ'},
    {'name': 'equipment_oxygen_concentrator',
    'ukr': 'Количество кислородных концентраторов'},
    {'name': 'centralized_oxygen_system',
    'ukr': 'Наличие системы централизованного снабжения кислорода'},
    {'name': 'equipment_ivl_oxygen_concentrator',
    'ukr': 'Количество кислородных генераторов к ИВЛ'}]
...
d3.csv("https://cdn.jsdelivr.net/gh/liga-net/few-projects/export_zapas.csv", type, function(error, data) {
if (error) throw error;
    var data = data.filter(k => k.name == filtered_value)

    var title = codesUkr.filter(k=>k.name == filtered_value)[0].ukr 
    title += '. Отчитались '+data.length+' больниц'
    ...
    if (filtered_value == 'robe_current') {
        ...
        var annotations = [{
            note: {
                title: 'Одна точка - одна больница'
            },
            color: 'black',
...
function makeBarChart(w,uploaded_data,id) {
    ...
    svg.append("text")
        ...
        .text('Количество закупленных концентраторов на Prozorro');
...
var chartOptions = {
    'id':'areachart',
    "title":"Количество выделенных и занятых коек",
    'subtitle':"",
    'col': ['amount','used'],
    'colors': ['#00441b','#EA3B3B'],
    'rowName':['Выделено','Занято']
}
...
var data = {
    labels: [
        "лечение COVID-19","вакцины","соцвыплаты","выплаты по безработице","образование","лекарство от COVID-19","больничные","доплаты для структур МВД","кислород для больниц", "выплаты заведениям НАМН"
],
series: [
    {label: 'Траты',values: [11800000000,9800000000,2900000000,1100000000,18300000,800000000,800000000,800000000,500000000,14000000]},
    {label: 'План',values: [17500000000,10500000000,2900000000,1100000000,1000000000,800000000,800000000,800000000,750000000,55000000]
}]
};
if (w>600) {
    var windowW = 600
