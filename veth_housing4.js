...
function makeBarChart(w) { 
    ...
    var margin = {top: 30, right: 37, bottom: 70, left: 150},
            ...
            if (value != null) {
                if (width > 450) {
                    var returnValue = '<tspan  x="'+geoPath.centroid(d)[0]+'" y="'+geoPath.centroid(d)[1]+'">'+name.replace(' область', '')+"</tspan>"+
                                    "<tspan x='"+geoPath.centroid(d)[0]+"' y='"+(geoPath.centroid(d)[1]+15)+"'>"+String((value/1000).toFixed(0))+' тыс.</tspan>'
                } else {
                    var returnValue = '<tspan  x="'+geoPath.centroid(d)[0]+'" y="'+geoPath.centroid(d)[1]+'">'+String((value/1000).toFixed(0))+' тыс.</tspan>'
                }
            }
            if (width > 450) {
