...
.domain([1, d3.max(values)])  // What's in the data, let's say it is percentage
.range([3, 30])  // Size in pixel

// Add legend: circles
//var valuesToShow = [d3.min(values),d3.max(values)/2, d3.max(values)]

var valuesToShow = [d3.max(values)/10,d3.max(values)/2,d3.max(values)]
var xCircle = 10
var xLabel = 170
var yCircle = 65
svg
...
var json = objects.map(d => d.properties)
