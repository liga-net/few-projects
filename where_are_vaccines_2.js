if(document.createStyleSheet) {
    document.createStyleSheet('https://cdn.jsdelivr.net/gh/liga-net/few-projects/where_are_vaccines.css');  
} else {
    var styles = "@import url('https://cdn.jsdelivr.net/gh/liga-net/few-projects/where_are_vaccines.css');";
...
var height = 80
var width = 200
d3.select("#maps_legend")
    .append('h4').text('Сколько сегодня есть вакцин в вашем городе, данные СНБО')
...
var json = objects.filter(d => d.regionName == region)
...
var text_json = json.filter((arr, index, self) =>
    index === self.findIndex((t) => (t.communityName === arr.communityName)))
...
cities_mer.selectAll("circle")
    ...
    .on("mouseover", function(d) {
        var koatuu = d.koatuu
        var hromada = json.filter(x => x.koatuu == koatuu)
        var address = d.properties.attributes.address
        ...
        if (hromada.length == 1){
            if (vaccines.length > 0) {
                var vaccines = vaccines.map(x => x.properties.daily.total.cumulative).reduce(reducer)
                tooltipBee.html('<h3>'+d.communityName+'</h3>'+'<p>Лікарня: '+d.entityName+'</p>'+'<p>Вакцин на складе: '+numberWithSpaces(vaccines)+'</p>'+'<p>Адреса: '+street+'</p>'+'<p>Телефон: '+d.registryPhone+'</p>') 
            } else {
                tooltipBee.html('<h3>'+d.communityName+'</h3>'+'<p>Лікарня: '+d.entityName+'</p>'+'<p>Адреса: '+street+'</p>'+'<p>Телефон: '+d.registryPhone+'</p>') 
            }
        } else {
            ...
            var html = '<h3>'+d.communityName+'</h3>'+'<p>Вакцин на складах: '+numberWithSpaces(vaccines)+'</p><h4>Лікарні:</h4>'
            ...
            var text_hromada = hromada.filter((arr, index, self) =>
                index === self.findIndex((t) => (t.entityName === arr.entityName)))
            text_hromada.forEach(function(value){
                vacc = vaccines_per_hospital.filter(v => v.properties.name == value.entityName)
                if (vacc.length > 0) {
                    html += '<p>'+value.entityName+': '+numberWithSpaces(vacc[0].properties.daily.total.cumulative)+' вакцин</p>'
                }
...
var text = cities_mer.selectAll("text")
    ...
    .html(function(d,i) {
        ...
        var x = json.filter(x => x.communityName == d.communityName)
        if (punkt.length > 0 && x.length> 2) {
            var punkt = punkt[0].replace('місто ','')
            
            if(d.communityName.includes(capitalizeFirstLetter(punkt).slice(0,5))) {
...
'objects':'https://cdn.jsdelivr.net/gh/liga-net/few-projects/objects.json',
'balance_objects':'https://cdn.jsdelivr.net/gh/liga-net/few-projects/balance_objects.json'
}
...
.html(function(d) {
    ...
    if (punkt.length > 0) {
        ...
        return '<td>'+capitalizeFirstLetter(punkt)+'</td>'+'<td>'+street+'</td>'+'<td>'+d.registryPhone+'</td>'+'<td>'+d.entityName+'</td>'
    }
