...
var rnboLinks = {
    ...
    'balance_objects':'balance_objects.json' */
...
function makeRegionMap(w) {
    ...
    cities_mer.selectAll("circle")
        ...
        .on("mouseover", function(d) {
            ...
            var street = getStreet(d.address.split(', '))
            var vaccines = getVaccines(koatuu)
            
            var html = '<h3>'+d.communityName+': <span style="font-weight: normal">'+vaccines+' вакцин</span></h3></p><h4>По лікарнях:</h4>'
            ...
            text_hromada.forEach(function(value){
                ...
                if (vacc.length > 0) {
                    html += '<p>'+value.entityName+': <b>'+numberWithSpaces(vacc[0].properties.daily.total.cumulative)+' вакцин</b></p>'
                }
            })
            tooltipBee.html(html) 
            var dist = d3.select(this)
    ...
    var text = cities_mer.selectAll("text")
        ...
        .html(function(d,i) {
            if (d.address.includes('місто')) {
                ...
                var vaccines_per_hospital = balance_objects.filter(k => k.properties.koatuu == d.koatuu)
                var hospitals = 0
                if (vaccines_per_hospital.length > 0) {
                    ...
                    var koatuu_vaccines = koatuu_vaccines.filter(x => x.koatuu == d.koatuu)
